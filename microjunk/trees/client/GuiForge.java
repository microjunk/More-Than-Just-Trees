...
import microjunk.trees.common.furnaces.ContainerForge;
...
public class GuiForge extends GuiContainer implements ICrafting
{
    private ContainerForge forgeContainer;
    private GuiTextField itemNameField;
    private InventoryPlayer field_82325_q;

    public GuiForge(InventoryPlayer par1, World world, int par3, int par4, int par5)
    {
        super(new ContainerForge(par1, world, par3, par4, par5, Minecraft.getMinecraft().thePlayer));
        this.field_82325_q = par1;
        this.forgeContainer = (ContainerForge)this.inventorySlots;
    }
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2)
    {
        ...
        this.fontRenderer.drawString(StatCollector.translateToLocal("container.forge"), 60, 6, 4210752);

        if (this.forgeContainer.maximumCost > 0)
        {
            int var3 = 8453920;
            boolean var4 = true;
            String var5 = StatCollector.translateToLocalFormatted("container.forge.cost", new Object[] {Integer.valueOf(this.forgeContainer.maximumCost)});

            if (this.forgeContainer.maximumCost >= 40 && !this.mc.thePlayer.capabilities.isCreativeMode)
            {
                var5 = StatCollector.translateToLocal("container.forge.expensive");
                var3 = 16736352;
            }
            else if (!this.forgeContainer.getSlot(2).getHasStack())
            {
                var4 = false;
            }
            else if (!this.forgeContainer.getSlot(2).canTakeStack(this.field_82325_q.player))
            {
                var3 = 16736352;
            }
    ...
    protected void keyTyped(char par1, int par2)
    {
        ...
        {
            this.forgeContainer.updateItemName(this.itemNameField.getText());
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3)
    {
        ...
        this.drawTexturedModalRect(var5 + 59, var6 + 20, 0, this.ySize + (this.forgeContainer.getSlot(0).getHasStack() ? 0 : 16), 110, 16);

        if ((this.forgeContainer.getSlot(0).getHasStack() || this.forgeContainer.getSlot(1).getHasStack()) && !this.forgeContainer.getSlot(2).getHasStack())
        {
