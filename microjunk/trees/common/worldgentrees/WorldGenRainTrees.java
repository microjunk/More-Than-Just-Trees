...
public class WorldGenRainTrees extends WorldGenerator
{
    ...
    public WorldGenRainTrees(boolean flag)
    {
            this(flag, 4, 0, 0, false);
    }

    public WorldGenRainTrees(boolean flag, int i, int j, int k, boolean flag1)
    {
            super(flag);
            field_48202_a = i;
            field_48201_c = j;
    ...
    public boolean generate(World world, Random random, int i, int j, int k)
    {
                    ...
                    for (int k1 = i - byte0; k1 <= i + byte0 && flag; k1++)
                    {
                            for (int i2 = k - byte0; i2 <= k + byte0 && flag; i2++)
                            {
                                    if (i1 >= 0 && i1 < 256)
                                    {
                                            ...
                                            if (i3 != 0 && i3 != Trees.waterLeaves.blockID && i3 != Block.dirt.blockID && i3 != Trees.waterLog.blockID)
                                            {
                                                    flag = false;
                                            }
                                    }
            ...
            if (j1 != Block.dirt.blockID && j1 != Block.grass.blockID && j1 != Block.sand.blockID || j >= 256 - l - 1)
            {
                    return false;
            }

            func_50073_a(world, i, j - 1, k, Trees.waterLog.blockID);
            byte byte1 = 3;
            int l1 = 0;
            ...
            for (int j2 = (j - byte1) + l; j2 <= j + l; j2++)
            {
                    ...
                    for (int k4 = i - i4; k4 <= i + i4; k4++)
                    {
                            ...
                            for (int k5 = k - i4; k5 <= k + i4; k5++)
                            {
                                    ...
                                    {
                                            setBlockAndMetadata(world, k4, j2, k5, Trees.waterLeaves.blockID, field_48199_d);
                                    }
                            }
                    }
            }
            ...
            for (int k2 = 0; k2 < l; k2++)
            {
                    ...
                    if (k3 != 0 && k3 != Trees.waterLeaves.blockID)
                    {
                            continue;
                    }

                    setBlockAndMetadata(world, i, j + k2, k, Trees.waterLog.blockID, field_48201_c);
            ...
            if (field_48200_b)
            {
                    for (int l2 = (j - 3) + l; l2 <= j + l; l2++)
                    {
                            ...
                            for (int l4 = i - j4; l4 <= i + j4; l4++)
                            {
                                    ...
                                    {
                                            if (world.getBlockId(l4, l2, j5) != Trees.waterLog.blockID)
                                            {
                                                    continue;
                                            }
